...
Compiler-
    time and deployment-time processing — Software tools can process annotation information to generate code, 
    XML files, and so forth.
Runtime processing — Some annotations are available to be examined at runtime.

Annotations can be applied to a programs declarations of classes, fields, methods, and other program elements.

The annotation appears first, often (by convention) on its own line, and may include elements with named or unnamed 
values:
@Author(
...
Suppose that a software group has traditionally begun the body of every class with comments providing important 
information:
...
The annotation type definition looks somewhat like an (interface) definition where the keyword (interface) is 
preceded by the @ character (@ = "AT" as in Annotation Type). Annotation types are, in fact, a form of interface, 
...
While its not required to use this annotation when overriding a method, it helps to prevent errors. 
...
(discussed in the lesson titled "Generics"). To suppress more than one category of warnings, use the following 
syntax:
